...
class Session:
    ...
    def append_vals_to_csv(self,name_file,vals):
        ...
        if not name_file_full in self.files:
            ...
            if not os.path.exists(name_file_full):
                ...
                writer_csv = csv.writer(fd, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
                writer_csv.writerow(keys)
        #else:
        writer_csv = csv.writer(self.files[name_file_full], delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
...
def is_pair_missing(pair):
    if m.vocabulary.get_id(pair[0])<0: return True
...
def is_quad_missing(quad):
    if m.vocabulary.get_id(quad[0])<0: return True
    if m.vocabulary.get_id(quad[2])<0: return True
    if m.vocabulary.get_id(quad[1][0])<0: return True
    if m.vocabulary.get_id(quad[3][0])<0: return True
    return False
...
def do_test_on_pair_3CosAdd(pairs_test,pairs_train,file_out):
    ...
    for p_test in pairs_test:
        for p_train in pairs_train:
            ...
            is_hit=False
            ids_question = {m.vocabulary.get_id(quad[0]),m.vocabulary.get_id(quad[2])}
            for z in quad[1]: ids_question.add(m.vocabulary.get_id(z))
            #print (ids_question)
...
def process_prediction(p_test_one,scores,score_reg,score_sim,file_out,p_train=[]):
    ids_max=np.argsort(scores)[::-1]
    ...
    if do_top5:
        ...
        for i in ids_max[:10]:
            if i == id_question: continue
            ans=m.vocabulary.get_word_by_id(i)
            if ans in set_exclude: continue
            cnt_reported+=1
            if score_sim is None:
                file_out.write("\tA:sc_ttl: {:.2f}\t{}\t".format(scores[i],ans))
            else:
                file_out.write("\tA:\tsc_class: {:.2f}\tsc_sim: {:.2f}\tsc_ttl: {:.2f}\t{}\t".format(score_reg[i],score_sim[i],scores[i],ans))
            if ans in p_test_one[1]:
                file_out.write ("YES\n")
            else:
                file_out.write ("NO\n")
            if cnt_reported>4: break
        file_out.write ("\n")
    else:
        for i in ids_max[:2]:
            if i == id_question: continue
...
#        model_regression = LogisticRegression(class_weight = 'balanced')
    ...
    if options["name_method"]=="SVMCos":
        model_regression = sklearn.svm.SVC(kernel=name_kernel, cache_size=1000, class_weight = 'balanced', probability=True)
...
def do_test_on_pair_3CosAvg(p_test,p_train,file_out):
    ...
    if len(vecs_a_prime)==0:
        for p_test_one in p_test:
            file_out.write("{}\t{}\t{}\n".format(p_test_one[0],p_test_one[1],"training MISSING"))
        return(len(p_test),0)
...
def run_category(pairs,name_dataset,name_category="not yet"):
    ...
    loo = sklearn.cross_validation.KFold(n = len(pairs), n_folds=len(pairs) // size_cv_test) 
    if need_subsample:
        ...
        for max_size_train in range (10,300,5):
            ...
            if finished:
                print("finished")
                break
            props_experiment={"vectors":m.name,"dataset":name_dataset,"method":options["name_method"],"category":name_category,"total":cnt_total,"correct":cnt_correct,"size_train":max_size_train}
            props_experiment.update(m.props)
        ...
        for train, test in loo:
            ...
            cnt_correct+=c
            cnt_total_correct+=c
        props_experiment={"vectors":m.name,"dataset":name_dataset,"method":options["name_method"],"category":name_category,"total":cnt_total,"correct":cnt_correct}
        props_experiment.update(m.props)
